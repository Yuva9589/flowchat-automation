...
function generateAnalyticsData(): DayData[] {
  ...
  for (let i = 89; i >= 0; i--) {
    ...
    // WhatsApp has highest engagement — bigger numbers
    const baseDMs = 60 + Math.floor(Math.random() * 100);
    const dms = baseDMs + Math.floor(Math.random() * 50);
    const clicks = Math.floor(dms * (0.7 + Math.random() * 0.25));
...
const topKeywords = [
  { keyword: "HI", dms: 512, percentage: 48 },
  { keyword: "PRICE", dms: 289, percentage: 27 },
  { keyword: "DEMO", dms: 267, percentage: 25 },
];

const topPosts = [
  { caption: "Auto-reply to 'HI' with welcome + catalog", dms: 512, keyword: "HI" },
  { caption: "'PRICE' triggers pricing PDF template", dms: 289, keyword: "PRICE" },
  { caption: "'DEMO' books a call via Calendly", dms: 267, keyword: "DEMO" },
];

/* ============= Main Component ============= */

export default function WhatsAppAnalytics({ isConnected }: Props) {
    ...
    <section id="analytics" className="scroll-mt-6">
                  ...
                  <>
                    ...
                    {[
                      ...
                      <button
                        ...
                        style={dateRange === opt.key ? { color: "#16a34a" } : {}}
                      >
                        {opt.label}
                        {dateRange === opt.key && (
                    ...
                    <button
                      ...
                      style={dateRange === "custom" ? { color: "#16a34a" } : {}}
                    >
                      <span>🎯</span>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all text-sm"
                        />
                      </div>
                      <div>
                        <label className="block text-[11px] font-semibold text-gray-700 mb-1 uppercase tracking-wider">
      ...
      {!isConnected ? (
        <div className="bg-white rounded-2xl p-8 border border-dashed border-gray-200 text-center">
          <p className="text-sm text-gray-500">
            Connect WhatsApp to see analytics 📊
          </p>
        </div>
      ) : (
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[11px] text-gray-500">By messages</span>
              </div>
              <div className="space-y-3">
                {topKeywords.map((k, i) => (
                  <div key={i}>
