...
export default function AdminDashboardPage() {
  ...
  const [loginOtpSentNotice, setLoginOtpSentNotice] = useState("");
  ...
  /* ============= Add New Admin Manager State (Gmail + Custom Password + Gmail Verification Code) ============= */
  ...
  const handleSendLoginOtp = async () => {
    ...
    setLoginError("");
    setLoginOtpSentNotice("");
    setOtpInput(""); // Ensure field is empty for user to type from Gmail
    ...
    try {
      ...
      setLoginOtpSentNotice(`📩 Verification Code sent to your Gmail inbox (${adminGmailInput})! Check your email.`);
    } catch (err: any) {
  ...
  const handleRequestResetOtp = async (e: React.FormEvent) => {
    ...
    setForgotErr("");
    setForgotMsg("");
    setForgotOtpInput(""); // Ensure field is empty
  ...
  const handleSendCodeToNewAdmin = async () => {
    ...
    setCodeSentNotice("");
    setAdminVerificationCodeInput(""); // Ensure field is empty for user input
  ...
  const handleAddAdminAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`✓ Admin Gmail ${createAdminEmail} Verified & Added to Whitelist with Full Access!`);
      setCreateAdminEmail("");
  ...
  /* Remove Admin Gmail from Whitelist in DB */
        ...
        <div className="max-w-md w-full bg-gray-900 rounded-3xl p-8 border border-gray-800 shadow-2xl space-y-6">
          ...
          {loginOtpSentNotice && (
            <div className="p-4 rounded-2xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-center space-y-1">
              <p className="text-xs font-bold text-emerald-400">{loginOtpSentNotice}</p>
            </div>
          )}
          ...
          <form onSubmit={handleAdminLoginSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                >
                  {isSendingLoginOtp ? "Sending..." : "Send OTP to Gmail →"}
                </button>
              </div>
              ...
              <form onSubmit={handleAddAdminAccount} className="space-y-4">
                ...
                <div className="p-4 bg-gray-950 rounded-2xl border border-gray-800 space-y-3">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    ...
                    >
                      {isSendingAddCode ? "Sending..." : "Send Verification Code to Gmail →"}
                    </button>
                  </div>
                  ...
                  <input
                    type="text"
                    required
                    value={adminVerificationCodeInput}
                    onChange={(e) => setAdminVerificationCodeInput(e.target.value)}
                    placeholder="Enter 6-digit verification code from Gmail"
                    autoComplete="off"
